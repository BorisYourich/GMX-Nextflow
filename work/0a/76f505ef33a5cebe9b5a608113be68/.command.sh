...
if [ ! -z  ]; then
    CPT="-t "`find /home/boris/GMX-Nextflow/RE/03// -name ".cpt"`
else
    CPT=""
fi
...
gmx grompp -f ${MDP}              -c ${GRO}              -r ${REF}              ${CPT}              -p ${TOP}              -n ${NDX}              -o /home/boris/GMX-Nextflow/RE/03/confident_bhaskara.tpr -quiet -maxwarn ${MAXWARN}
